...
% Version :  1.2
% Function:  Synchronisation_Gait_Analysis_Left GUI
% Authors  :  Safwan Al-Qadhi, Dr. Mehmet Eylem Kirlangic

function varargout = Synchronisation_Gait_Analysis_Left(varargin)
%SYNCHRONISATION_GAIT_ANALYSIS_LEFT M-file for Synchronisation_Gait_Analysis_Left.fig
%      SYNCHRONISATION_GAIT_ANALYSIS_LEFT, by itself, creates a new SYNCHRONISATION_GAIT_ANALYSIS_LEFT or raises the existing
%      singleton*.
%
%      H = SYNCHRONISATION_GAIT_ANALYSIS_LEFT returns the handle to a new SYNCHRONISATION_GAIT_ANALYSIS_LEFT or the handle to
%      the existing singleton*.
%
%      SYNCHRONISATION_GAIT_ANALYSIS_LEFT('Property','Value',...) creates a new SYNCHRONISATION_GAIT_ANALYSIS_LEFT using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to Synchronisation_Gait_Analysis_Left_OpeningFcn.  This calling syntax produces a
%
%      SYNCHRONISATION_GAIT_ANALYSIS_LEFT('CALLBACK') and SYNCHRONISATION_GAIT_ANALYSIS_LEFT('CALLBACK',hObject,...) call the
%      local function named CALLBACK in SYNCHRONISATION_GAIT_ANALYSIS_LEFT.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help Synchronisation_Gait_Analysis_Left

% Last Modified by GUIDE v2.5 18-May-2019 13:20:52
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Synchronisation_Gait_Analysis_Left_OpeningFcn, ...
                   'gui_OutputFcn',  @Synchronisation_Gait_Analysis_Left_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
%% --- Executes just before Synchronisation_Gait_Analysis_Left is made visible.
function Synchronisation_Gait_Analysis_Left_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for Synchronisation_Gait_Analysis_Left
...
% UIWAIT makes Synchronisation_Gait_Analysis_Left wait for user response (see UIRESUME)
% uiwait(handles.figure1);

%% --- Outputs from this function are returned to the command line.
function varargout = Synchronisation_Gait_Analysis_Left_OutputFcn(hObject, eventdata, handles)
...
Gait1_axes(1)=handles.axes1;
Gait1_axes(2)=handles.axes2;
Gait1_axes(3)=handles.axes3;
Gait1_axes(4)=handles.axes4;
Gait1_axes(5)=handles.axes5;
Gait1_axes(6)=handles.axes6;
Gait1_axes(7)=handles.axes7;
Gait1_axes(8)=handles.axes8;

[directory_name,Patient_ID_file]=find_folder();

if isdir(directory_name) && ~isequal(directory_name,'')
    
look_for_this_file_steplength='Step_Length_1.txt';
...
cla(Gait1_axes(1),'reset')
axes(Gait1_axes(1));
set(Gait1_axes(1),'XMinorTick','on')
plot(Time_steplength,sg_steplength)
grid on

else
cla(Gait1_axes(1),'reset')
set(Gait1_axes(1),'XGrid','on','YGrid','on')
Time_steplength='';
sg_steplength='';
end 

look_for_this_file_Ankel='Ankle_left.txt';
...
cla(Gait1_axes(2),'reset')
axes(Gait1_axes(2));
set(Gait1_axes(2),'XMinorTick','on')
plot(Y1(1:end-1),diff(Y1))
grid off
else
cla(Gait1_axes(2),'reset')
set(Gait1_axes(2),'XGrid','on','YGrid','on')
end
...
look_for_this_file_Knee_Angle='Knee_Angle_left.txt';
...
cla(Gait1_axes(3),'reset')
axes(Gait1_axes(3));
set(Gait1_axes(3),'XMinorTick','on')
plot(Time_Knee_Angle,sg_Knee_Angle)
grid on

else
    cla(Gait1_axes(3),'reset')
    set(Gait1_axes(3),'XGrid','on','YGrid','on')
    cla(Gait1_axes(6),'reset')
    set(Gait1_axes(6),'XGrid','on','YGrid','on')
    sg_Knee_Angle='';
end 

look_for_this_file_Hip_Knee_Angle='Hip_left.txt';
...
cla(Gait1_axes(4),'reset')
axes(Gait1_axes(4));
set(Gait1_axes(4),'XMinorTick','on')
plot(Time_Hip_Knee_Angle,sg_Hip_Knee_Angle)
grid on

else
    cla(Gait1_axes(4),'reset')
    set(Gait1_axes(4),'XGrid','on','YGrid','on')
    cla(Gait1_axes(7),'reset')
    set(Gait1_axes(7),'XGrid','on','YGrid','on')
    sg_Hip_Knee_Angle='';
end 

look_for_this_file_Knee_Ankle_Angle='Knee_Ankle_Angle_left.txt';
...
cla(Gait1_axes(5),'reset')
axes(Gait1_axes(5));
set(Gait1_axes(5),'XMinorTick','on')
...
% set(handles.patient_number,'String','');
    ...
    cla(Gait1_axes(5),'reset')
    set(Gait1_axes(5),'XGrid','on','YGrid','on')
    cla(Gait1_axes(8),'reset')
    set(Gait1_axes(8),'XGrid','on','YGrid','on')
    sg_Knee_Ankle_Angle='';
end 
...
VarNam_Steplength={'Gait Left cadence','Gait Left mean Stride in m','Gait Left std. Stride',...
       'Gait Left mean Step Left in m','Gait Left mean Step Left in m','Gait Left std. Step Left',...
       'Gait Left std. Step Left','Gait Left meter per minute'};
...
Gait1_axes(9)=handles.axes9; 
Gait1_axes(10)=handles.axes10;
Gait1_axes(11)=handles.axes11;

if isempty(sg_Knee_Angle)|| isempty(sg_Hip_Knee_Angle ) || isempty(sg_Knee_Ankle_Angle)  
    cla(Gait1_axes(9),'reset')
    set(Gait1_axes(9),'XGrid','on','YGrid','on')
    cla(Gait1_axes(10),'reset')
    set(Gait1_axes(10),'XGrid','on','YGrid','on')
    cla(Gait1_axes(11),'reset')
    set(Gait1_axes(11),'XGrid','on','YGrid','on')
...
VarNam_Knee_Angle={'Gait Left Knee Angle EVD mu','Gait Left Knee Angle EVD sigma',...
       'Gait Left Knee Angle EVD mean','Gait Left Knee Angle EVD variance'};
...
VarNam_Hip_Knee_Angle={'Gait Left Hip Knee Angle EVD mu','Gait Left Hip Knee Angle EVD sigma',...
       'Gait Left Hip Knee Angle EVD mean','Gait Left Hip Knee Angle EVD variance'};
...
%% Statistik Knee_Angle_Ankle_Left
...
VarNam_Knee_Ankle_Angle={'Gait Left Knee Ankle Angle EVD mu','Gait Left Knee Ankle Angle EVD sigma',...
       'Gait Left Knee Ankle Angle EVD mean','Gait Left Knee Ankle Angle EVD variance'};
...
if X_end <= X_start
    ...
    set(Gait1_axes(val),'Xlim',[X_start Left_measure_Time+2]);
    
elseif isnan(X_end)
    set(hObject, 'String', 0);
    errordlg('Input must be a number','Error');
    X_end=str2double(get(hObject,'String'));
    set(Gait1_axes(val),'Xlim',[X_start X_end]);
...
frame = getframe(Synchronisation_Gait_Analysis_Left);
...
%                     mkdir(saveFilePath,Name); Newpath=strcat(saveFilePath,Name)
                    fid = fopen(strcat(saveFilePath,Name,'_GaitAnalysisLeft.txt'), 'wt');
                    ...
                    imwrite(frame.cdata,strcat(saveFilePath,Name,'_GaitAnalysisLeft.bmp'));
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
GUI_HANDLE=Synchronisation_Gait_Analysis_Left();
...
switch Qd
    ...
    case str2
        close(Synchronisation_Gait_Analysis_Left)
...
%         close force
    ...
    fclose('all');
    disp(' ');
    disp('***** END Synchronisation and Gait Analysis - Left GUI *****');
    disp(' ');
    disp(' ');
    clear all;
